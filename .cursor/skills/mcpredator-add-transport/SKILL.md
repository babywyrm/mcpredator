---
name: mcpredator-add-transport
description: >-
  Add a new transport type to mcpredator alongside SSE, HTTP, and ToolServer.
  Use when adding support for a new MCP transport protocol like DPoP, mTLS,
  or a custom gateway.
---

# Add a New mcpredator Transport
...
In `mcpredator/core/session.py`, add a new class:
...
In `mcpredator/cli.py`, add arguments:
...
In `mcpredator/scanner.py` `scan_target()`, add label detection:
